var laya = (function () {
	...
	class GPUSkinningPlayer {
	    ...
	    onRenderUpdate(context, transform, render) {
	        ...
	        render._shaderValues.setVector(GPUSkinningPlayerResources.shaderPorpID_GPUSkinning_FrameIndex_PixelSegmentation, new Vector4$1(this.__frameIndex, this.playingClip.pixelSegmentation, 0, 0));
	    }
	...
	class GPUSkining {
	    static async InitAsync() {
	        ...
	        Shader3D$3.addInclude("GPUSkinningInclude.glsl", GPUSkinningIncludegGLSL);
	...
	class GPUSkining$1 {
	    static async InitAsync() {
	        ...
	        LayaExtends_Laya3D.Init();
	        Laya3D.SKING_MESH = "SKING_MESH";
	        var createMap = LoaderManager$1.createMap;
	        createMap["skinlm"] = [Laya3D.SKING_MESH, GPUSkiningMesh._parse];
	        var parserMap = Loader$1.parserMap;
	        parserMap[Laya3D.SKING_MESH] = this._loadMesh;
	    }
	    static _loadMesh(loader) {
	        loader.on(Event$1.LOADED, null, this._onMeshLmLoaded, [loader]);
	        loader.load(loader.url, Loader$1.BUFFER, false, null, true);
	    }
	    static _onMeshLmLoaded(loader, lmData) {
	        loader._cache = loader._createCache;
	        var mesh = GPUSkiningMesh._parse(lmData, loader._propertyParams, loader._constructParams);
	...
	GPUSkining$1.EXT_SKING_MESH = "skinlm";
	GPUSkining$1.resRoot = "res3d/Conventional/";
	window['GPUSkining'] = GPUSkining$1;
