...
void main_normal()
{
	...
	vec4 mainTexture = u_DiffuseColor;
	#ifdef DIFFUSEMAP
		mainTexture = texture2D(u_DiffuseTexture, v_Texcoord0);
	#endif 
	...
	// 灯光和法线角度， 值加深
	float angleExp = clamp(pow(angle, u_ColorRange), 0.0, u_ColorDeep);
	...
	vec3 emissive = (
						(
							...
							+
							(u_ShadowColor.rgb * angleExp)
						)
						+
						(
							(mainTexture.rgb * u_DiffuseColor.rgb)
							*
							ambient
						)
					);
